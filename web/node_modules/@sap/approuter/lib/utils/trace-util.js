...
exports.traceBackendRequest = function (tracer, url, requestOptions, destination) {
  if (tracer && tracer.isLoggingLevel && tracer.isLoggingLevel('debug')) {
    if (destination) {
      const optsCA = requestOptions.ca;
      const agentCA  = requestOptions.agent.options && requestOptions.agent.options.ca;
      tracer.debug('Request to backend. Destination details:', 'name:', destination.name, 'url:', destination.url, 'opts ca', !!optsCA, 'agentCA:',!!agentCA);
    }
