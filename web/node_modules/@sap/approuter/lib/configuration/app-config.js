...
function generateErrorPageMap(configuration) {
  return (configuration.errorPage || []).reduce(function (result, errorRoute) {
    ...
    arr.forEach(function (status) {
      result.set(status, {file: errorRoute.file, path: errorRoute.path});
    });
    return result;
  }, new Map());
}
...
function processCors(configuration) {
  ...
  configuration.cors.forEach(function (currentCors) {
    if (currentCors.hostPattern) {
      currentCors.hostPattern = configurationUtils.constructRegExp(currentCors.hostPattern);
      if (!safeRegex(currentCors.hostPattern)) {
        logger.warning('Cors configuration with hostPattern ', currentCors.hostPattern, ' is vulnerable to ReDoS attacks');
      }
    }
