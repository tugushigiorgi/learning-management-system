...
async function getDestination (req, res, options, next){
  ...
  return destinationTokenHandler.retrieveDestination(options, async function (err, result) {
    ...
    sessionExt.update(req.session, function(// eslint-disable-next-line
        session) {
      if (!session.user.destinations){
        session.user.destinations = {};
      }
      if (!session.user.destinationUserExchangeToken){
        session.user.destinationUserExchangeToken = {};
      }
      if (!session.user.destinations[key]){
        session.user.destinations[key] = {};
      }
      if (result.authToken && !session.user.destinations[key].authToken){
        session.user.destinations[key].authToken = {};
      }
      if (result.authToken) {
        session.user.destinations[key].authToken = result.authToken;
      }
      session.user.destinations[key].expireDate = result.expireDate;
      session.user.destinations[key].destinationConfiguration = result.destination;

      if (result.userExchangeToken) {
        if (req.destinationKey){
          if (!session.user.destinationKey){
            session.user.destinationKey = {};
          }
          session.user.destinationKey[req.destinationKey] = { destinationUserExchangeToken: result.userExchangeToken};
        } else {
          session.user.destinationUserExchangeToken = result.userExchangeToken;
        }
      }
    },next);
  });
