...
const corsUtils = require('../utils/cors-utils');
...
function getCorsDefinition (req) {
  ...
  if (envCors && !xsappCors)
  {
    return _.find(envCors, function (envCors) {
      return corsUtils.matchTheHostAndUriPattern(req, envCors);
    });
  }
  if (!envCors && xsappCors)
  {
    return _.find(xsappCors, function (xsappCors) {
      return corsUtils.matchTheHostAndUriPattern(req, xsappCors);
    });
  }
  if (envCors && xsappCors)
  {
...
function fillDefaultsCorsConfig(corsConfig) {
  ...
  corsConfigWithDefaults.hostPattern = corsConfig.hostPattern;
